...
class Generator extends \yii\xgii\Generator
{
    ...
    /**
     * Validates the [[tableName]] attribute and create datatable.
     */
    public function validateTableName()
    {
        ...
        if ($this->isReservedKeyword($this->tableName) || $this->isReservedKeyword($className)) {
            $this->addError('tableName', "数据表 '$table'含有PHP保留字符.");
            return;
        }
        ...
        if($this->tableExisted($db, $this->tableName)) {
            ...
            try {
                ...
                $this->addError('tableName', "Table '{$this->tableName}' 数据表创建事务操作失败，已回滚");
            }
        } else {
            $db->createCommand()->createTable($this->tableName,$columns,$tableOptions)->execute();
        }
    ...
    public function checkTableName($tableName) {
        //常规检查
        if (strpos($tableName, '*') !== false && substr_compare($tableName, '*', -1, 1)) {
            return false;
        }
        //保留字段
        $className = $this->generateClassName($tableName);
        if ($this->isReservedKeyword($tableName) || $this->isReservedKeyword($className)) {
            return false;
        }
    }
    /**
