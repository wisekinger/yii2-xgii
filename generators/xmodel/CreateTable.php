...
class Generator extends \yii\xgii\Generator
{
    ...
    protected function createDbTable($overWrite = true)
    {
        if (!isset($this->tableName)) {
            return CREATE_TABLE_FLAG_FAILURE;
        }
        $db = $this->getDbConnection();
        if ($db === null) {
            return CREATE_TABLE_FLAG_FAILURE;
        ...
        if ($overWrite == false) {
            $this->tableName.=$this->tableName.time();
        }
        if ($db->createCommand('CREATE TABLE ').$this->tableName) {
            return CREATE_TABLE_FLAG_SUCCESS;
        }
        else {
            return CREATE_TABLE_FLAG_FAILURE;
        }
